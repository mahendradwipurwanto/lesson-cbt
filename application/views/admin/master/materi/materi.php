					...
					<div class="col-sm mb-2 mb-sm-0">
						...
						<div class="tom-select-custom">
							<select class="js-select form-select form-select-sm" id="filter_status" autocomplete="off"
								data-hs-tom-select-options='{"placeholder": "Semua status"}'>
...
<!-- Modal -->
<div class="modal fade" id="mdlMateriDetail" tabindex="-1" aria-labelledby="mdlDeleteLabel" aria-hidden="true">
	<div class="modal-dialog modal-dialog-centered modal-xl">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="mdlDeleteLabel">Detail Materi</h5>
				<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
			</div>
			<div class="modal-body" id="modalMateriContent">
			</div>
		</div>
	</div>
</div>
...
<!-- Modal -->
<div class="modal fade" id="mdlMateriDelete" tabindex="-1" aria-labelledby="mdlDeleteLabel" aria-hidden="true">
	<div class="modal-dialog modal-dialog-centered modal-sm">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="mdlDeleteLabel">Hapus Materi</h5>
				<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
			</div>

			<div class="modal-body">
				<div class="mb-2">Apakah kamu yakin ingin menghapus soal : <span class="mdlMateriDelete_Label"
			...
			<div class="modal-footer">
				<form action="<?= site_url('master/deleteMateri')?> " method="post" class="js-validate need-validate"
					novalidate>
					<input type="hidden" name="id" class="mdlMateriDelete_id">
...
<!-- Modal -->
<div class="modal fade" id="mdlMateriArsip" tabindex="-1" aria-labelledby="mdlArsipLabel" aria-hidden="true">
	<div class="modal-dialog modal-dialog-centered modal-sm">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="mdlArsipLabel">Arsipkan Materi</h5>
				<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
			</div>

			<div class="modal-body">
				<div class="mb-2">Apakah kamu yakin ingin mengarsipkan soal : <span class="mdlMateriArsip_Label"
						style="font-weight: bold;"></span></div>
			</div>
			...
			<div class="modal-footer">
				<form action="<?= site_url('master/arsipMateri')?> " method="post" class="js-validate need-validate"
					novalidate>
					<input type="hidden" name="id" class="mdlMateriArsip_id">
					<button type="button" class="btn btn-outline-secondary btn-sm"
						data-bs-dismiss="modal">Batal</button>
...
<script>
	var table = $('#dataTable').DataTable({
		...
		'ajax': {
			'url': "<?= site_url('ajax/master/getAjaxMateri')?>",
			'data': function (d) {
	...
	const showMdlMateriDetail = id => {
		$('#mdlChecked_id').val(id);

		$("#modalMateriContent").html(
			`<center class="py-5"><span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Sedang memuat ...</center>`
		);

		$('#mdlMateriDetail').modal('show')

		jQuery.ajax({
			url: "<?= site_url('ajax/master/getDetailMateri') ?>",
			type: 'POST',
			data: {
				user_id: id
			},
			success: function (data) {
				$("#modalMateriContent").html(data);
			}
		});
	}

	const showMdlMateriDelete = function (id, judul) {
		$('.mdlMateriDelete_id').val(id);
		$('.mdlMateriDelete_Label').html(`#${judul}`);
		$('#mdlMateriDelete').modal('show')
	}

	const showMdlMateriArsip = function (id, judul) {
		$('.mdlMateriArsip_id').val(id);
		$('.mdlMateriArsip_Label').html(`#${judul}`);
		$('#mdlMateriArsip').modal('show')
	}
